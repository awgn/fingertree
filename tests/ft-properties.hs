...
(~=) :: (Eq a, Measured [a] a, Valid a) => Seq a -> [a] -> Bool
...
prop_fmapWithPos xs =
    fmapWithPos f xs ~= zipWith f (inits xs_list) xs_list
  where
    f = (,)
    xs_list = toList xs
...
prop_traverseWithPos xs =
    evalM (traverseWithPos f xs) ~= evalM (traverse (uncurry f) (zip (inits xs_list) xs_list))
  where
    f xs y = do
        n <- step
        return (xs, n, y)
